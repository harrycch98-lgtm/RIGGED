...
// ===== END WEBSOCKET INTEGRATION ====="use strict";
  ...
  if (createLobbyButton) createLobbyButton.addEventListener("click", createMultiplayerLobby);
  ...
  function beginSelectedGame(options = {}) {
    ...
    startGame();
    setSoundEnabled(true, { announce: false, fade: 1.2 });
    if (!loopStarted) {
      loopStarted = true;
  ...
  if (speedControl) {
    speedControl.addEventListener("click", (event) => {
      ...
      if (!button || !speedControl.contains(button)) return;
      if (multiplayerState.enabled && Number(button.dataset.speed) !== 1) {
        setGameSpeed(1);
        showToast("Multiplayer is locked to 1x speed.", "compact");
        return;
      }
  ...
  if (difficultyInput) {
    difficultyInput.addEventListener("change", refreshDifficultyNote);
  ...
  if (matchModeInput) {
    matchModeInput.addEventListener("change", refreshModeNote);
    refreshModeNote();
  }
  ...
  function ensureCrazySdk() {
    ...
    crazySdkPromise = new Promise((resolve) => {
      ...
      if (existing) {
        Promise.resolve(window.CrazyGames?.SDK?.init?.()).catch(() => null).finally(() => resolve(getCrazyGameSdk()));
        return;
      }
  ...
  function refreshMultiplayerUi() {
    ...
    multiplayerInvite.textContent = `Room ${multiplayerState.roomId}`;
    if (copyInviteButton) copyInviteButton.disabled = !multiplayerState.inviteUrl;
    if (createLobbyButton) createLobbyButton.textContent = multiplayerState.host ? "New Lobby" : "Create";
    syncMultiplayerSpeedControls();
    renderLobbyParty();
    updateLobbyStartButtons();
  }

  function syncMultiplayerSpeedControls() {
    if (!speedControl) return;
    speedControl.querySelectorAll("[data-speed]").forEach((button) => {
      const locked = multiplayerState.enabled && Number(button.dataset.speed) !== 1;
      button.disabled = locked;
      button.title = locked ? "Multiplayer is locked to 1x speed" : "";
    });
    if (multiplayerState.enabled && gameSpeed !== 1) {
      gameSpeed = 1;
      speedControl.dataset.speed = "1";
      speedControl.querySelectorAll("[data-speed]").forEach((button) => {
        button.classList.toggle("is-active", Number(button.dataset.speed) === 1);
      });
    }
  }
  ...
  async function copyInviteLink() {
    if (!multiplayerState.inviteUrl) return;
    try {
      const sdkGame = await ensureCrazySdk();
      if (sdkGame?.copyToClipboard) sdkGame.copyToClipboard(multiplayerState.inviteUrl);
      else await navigator.clipboard.writeText(multiplayerState.inviteUrl);
      showToast("Invite link copied.", "compact");
    } catch {
      multiplayerInvite.textContent = multiplayerState.inviteUrl;
      showToast("Copy blocked. Link shown in lobby box.", "compact");
    }
  }
  ...
  async function detectMultiplayerInvite() {
    ...
    if (sdkGame.inviteParams) applyJoinedRoom(sdkGame.inviteParams);
  ...
  function registerCrazyRoomJoinListener(sdkGame) {
    const listener = (inviteParams) => applyJoinedRoom(inviteParams);
  ...
  function startGame() {
    ...
    const count = Math.min(Number(playerCountInput.value), FACTIONS.length);
    const partyOrder = [selectedParty, ...FACTIONS.map((_, index) => index).filter((index) => index !== selectedParty)].slice(0, count);
    players = partyOrder.map((factionIndex, id) => {
      const faction = factionForMenu(factionIndex);
      const leaderProfile = id === HUMAN
      ...
      factionIndex,
      leaderProfile,
      aiPersonality: id === HUMAN ? AI_PERSONALITIES[0] : AI_PERSONALITIES[(id - 1) % AI_PERSONALITIES.length],
      cash: (id === HUMAN ? 15000 : 14000) + botCashBonus({ id }),
      heat: 0,
      locked: 0,
      leaderDeaths: 0,
      assassinDay: -1,
      ...
      basePickDelay: id === HUMAN ? 0 : 1.2 + Math.random() * 6.5,
      homeBase: -1,
      mainBaseLevel: 0,
      talents: {},
      hypeNext: false,
  ...
  function setGameSpeed(speed) {
    const allowed = [1, 2, 4, 10];
    if (multiplayerState.enabled) speed = 1;
    gameSpeed = allowed.includes(speed) ? speed : 1;
    if (speedControl) {
      speedControl.querySelectorAll("[data-speed]").forEach((button) => {
        button.classList.toggle("is-active", Number(button.dataset.speed) === gameSpeed);
      });
      speedControl.dataset.speed = String(gameSpeed);
    }
    showToast("Game speed: " + gameSpeed + "x");
  }
  ...
  function update(dt) {
    if (phase === "base") {
      ...
      players.forEach((player) => {
        if (player.id === HUMAN || player.homeBase >= 0) return;
    ...
    players.forEach((player) => {
      ...
      if (player.id !== HUMAN) tickAi(player, dt);
    });
    tickMissions(dt);
  ...
  function syncSoundButtons() {
    newsSoundButtons.forEach((button) => { button.textContent = soundOn ? "Sound On" : "Sound Off"; });
  ...
  function drawActiveTacticalOverlays() {
    players.forEach((player) => {
      if (!player.action) return;
      const state = states[player.action.state];
      const progress = 1 - player.action.left / player.action.total;
      const x = state.cx;
      const y = state.cy;
      if (player.action.type === "speech") {
        drawSpeechBroadcast(x, y, player);
        drawCountdownBar(x - 28, y - 32, 56, progress, factionVisual(player).glow, player.action.left);
      }
    });
    missions.forEach((mission) => {
      const state = states[mission.state];
  ...
  function botCashBonus(player) {
    if (!player || player.id === HUMAN) return 0;
  ...
  function botActionDiscount(player) {
    if (!player || player.id === HUMAN) return 1;
    const difficulty = difficultyInput?.value || "medium";
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape") { event.preventDefault(); settingsOpen ? closeSettingsPanel() : openSettingsPanel(); return; }
